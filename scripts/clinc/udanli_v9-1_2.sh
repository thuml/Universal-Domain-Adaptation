...
for seed in  $seeds; do
    for lr in $lrs; do
        python nlp/udanli_v9-1.py --config configs/nlp/udanli/udanli-clinc-opda_0.4.yaml --lr $lr --seed $seed --num_nli_sample 2
    done
done
...
for seed in  $seeds; do
    for lr in $lrs; do
        python nlp/udanli_v9-1.py --config configs/nlp/udanli/udanli-clinc-opda_0.5.yaml --lr $lr --seed $seed --num_nli_sample 2
    done
done
