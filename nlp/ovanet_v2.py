...
def main(args, save_config):
    ...
    ## LOGGINGS ##
    log_dir = f'{args.log.output_dir}/{args.dataset.name}/ovanet_different_lr/common-class-{args.dataset.num_common_class}/{args.train.seed}/{args.train.lr}'
    ...
    ## OPTIMIZER & SCHEDULER ##
    optimizer_model = optim.AdamW(model.model.parameters(), lr=args.train.lr / 10)
    optimizer_classifier = optim.AdamW(list(model.C1.parameters()) + list(model.C2.parameters()), lr=args.train.lr)
    ...
    lr_scheduler_classifier = get_scheduler(
        name=args.train.lr_scheduler_type,
