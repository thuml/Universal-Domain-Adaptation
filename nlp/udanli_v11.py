...
from utils.data import get_udanli_datasets_v10, ForeverDataIterator
...
def main(args, save_config):
    ...
    ## LOGGINGS ##
    log_dir = f'{args.log.output_dir}/{args.dataset.name}/udanli_v11-1/udanli-{args.train.adv_weight}-{args.train.ent_weight}-{args.num_nli_sample}/common-class-{args.dataset.num_common_class}/{args.train.seed}/{args.train.lr}'
    ...
    ## GET DATASETS ##
    nli_data, adv_data, ent_data, train_data, val_data, test_data, source_test_data = get_udanli_datasets_v10(root_path=args.dataset.root_path, task_name=args.dataset.name, seed=args.train.seed, num_common_class=args.dataset.num_common_class, num_nli_sample=args.num_nli_sample)
    ...
    if args.train.train:
        ...
        for current_epoch in range(1, args.train.num_train_epochs+1):
            ...
            for current_step in tqdm(range(num_step_per_epoch), desc=f'TRAIN EPOCH {current_epoch}'):
                ...
                ## total loss
                loss = (1-args.train.adv_weight) * nli_loss + args.train.adv_weight * adv_loss + args.train.ent_weight * entropy
