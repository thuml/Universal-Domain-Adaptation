...
def main(args, save_config):
    ...
    for class_index in range(num_source_labels):
        # entailment samples
        dataset_per_class = train_data.filter(lambda sample : sample[coarse_label] == class_index)
        class2dataset[class_index] = dataset_per_class
    ...
    with open(output_file, 'w') as f:
        for first_sample in tqdm(train_data, desc='Generating NLI samples'):
            ...
            entail_dataset = class2dataset.get(label)
            ...
            contradiction_dataset = class2dataset.get(-label)
